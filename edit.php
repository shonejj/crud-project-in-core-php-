...
$qry2=mysqli_query($conn,"select * from upload ");
if($qry2)
{
    $_SESSION['id']=$id;
...
{
echo '<script>alert(" deleted process was not completed there is a error please try again ")</script>';
}
}
...
<div class="flex">
 ...
 <?php  
  ...
  $qry=mysqli_query($conn,"select * from upload where uname='$uname'");
...
<div>
  <img src="<?php echo "$folder".$row['img']; ?>" width="200px" height="200px" >
    ...
    <p class="p1"><?php   echo $row['discription'];  ?></p> 
    <p class="p1"><?php   echo $row['content'];  ?></p>
    <a href="post.php?id=<?php echo $row['id']?>" class="a1">Read more</a></br>
    <a id="myBtn" href='edit.php?id=<?php echo $row['id']?>' class="a1">Edit</a></br> 
    <hr>
	<p class="p1"><?php  echo $row['date']," | by " , $row['uname']; ?></p>
	</div>
</div>
<?php 
  }
